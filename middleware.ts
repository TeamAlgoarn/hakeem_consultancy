...
export function middleware(req: NextRequest) {
  ...
  // Protect all /billing routes EXCEPT /billing/login
  if (pathname.startsWith('/billing') && !pathname.startsWith('/billing/login')) {
...
// Apply middleware only to /billing routes
export const config = {
  matcher: ['/billing/:path*'],
};