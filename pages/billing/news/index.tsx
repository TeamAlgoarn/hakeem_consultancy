...
// --- Supabase API helpers (replaces localStorage) ---
async function apiList(): Promise<News[]> {
  const r = await fetch('/api/news', { cache: 'no-store' });
  if (!r.ok) throw new Error('Failed to load news');
  return r.json();
}

async function apiUpsert(payload: { id?: string; title: string; body: string }) {
...
async function apiDelete(id: string) {
  const r = await fetch(`/api/news?id=${encodeURIComponent(id)}`, { method: 'DELETE' });
  const data = await r.json();
...
export default function NewsAdmin() {
  ...
  useEffect(() => {
    (async () => {
      try {
        // optional: request persistent storage (harmless even when using Supabase)
        if (navigator.storage?.persist) {
          const granted = await navigator.storage.persist();
          console.log('Persistent granted:', granted);
        }
        ...
        alert(e?.message || 'Failed to load news');
      }
    })();
  }, []);
  ...
  const upsert = async () => {
    ...
    try {
      if (editingId) {
        await apiUpsert({ id: editingId, title: title.trim(), body: body.trim() });
        setItems(await apiList());
        reset();
        return;
      }

      await apiUpsert({ title: title.trim(), body: body.trim() });
      setItems(await apiList());
      reset();
    } catch (e: any) {
              ...
              <ul className="space-y-6">
                {filtered.map((n) => (
                  <li key={n.id} className="group relative">
                    ...
                    <h4 className="pr-28 font-semibold text-gray-900">{n.title}</h4>
                    <p className="mt-1 text-sm leading-relaxed text-gray-600">{n.body}</p>
                    <p className="mt-2 text-xs text-gray-500">Updated: {n.updated_at.slice(0, 10)}</p>
