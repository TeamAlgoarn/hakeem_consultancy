...
//     {/* Card */}
//     <div className="relative mx-auto mt-3 w-[92%] max-w-sm rounded-2xl overflow-hidden border border-white/10 bg-slate-900/90 shadow-2xl">
//       {/* Top row: logo + close */}
...
//           href="/"
//           onClick={() => setMenuOpen(false)}
//           className="block text-white/90 hover:text-white px-3 py-2 rounded-lg hover:bg-white/10"
//         >
//           HOME
//         </Link>
//         <Link
//           href="/about"
//           onClick={() => setMenuOpen(false)}
//           className="block text-white px-3 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500"
//         >
//           ABOUT US
//         </Link>
//         <Link
...
//           href="/counselling"
//           onClick={() => setMenuOpen(false)}
//           className="block text-center bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg font-medium mt-2"
//         >
//           Book Free Counselling
//         </Link>
//       </div>
...
// )}
//         {/* // <div className="lg:hidden bg-slate-900/95 backdrop-blur-sm px-4 py-4 space-y-2 border-b border-slate-700 shadow-xl">
//         // <div className="lg:hidden bg-black/40 backdrop-blur-md px-4 py-4 space-y-2 border-b border-transparent shadow-xl">
//         //   <div className="flex justify-center mb-4">
//         //     <GoogleRatingComponent />
//         //   </div> */}
// {/* 
...
export default function Header() {
  ...
  // active states: exact match or subpath
  const _pathname = usePathname() ?? '';  // <-- normalize to empty string

const isActive = (href: string) =>
  _pathname === href || _pathname.startsWith(href + '/');
  ...
  //   }`;
const itemClassMobile = (href: string) =>
  `block px-3 py-2 rounded-lg text-sm font-medium transition-all ${
    isActive(href)
      ? 'bg-indigo-600 text-white'                        // MOBILE active: indigo bar
      : 'text-white/90 hover:text-white hover:bg-white/10'
  }`;
  const itemClassDesktop = (href: string) =>
  `px-3 py-1.5 rounded-full text-sm font-medium transition-all ${
    isActive(href)
      ? 'text-white font-semibold'                        // DESKTOP active: no indigo bg
      : 'text-white/90'
  } hover:bg-white/10`;
  // Optional: support #hash smooth-scroll (currently all your links are pages)
  const handleHashOrRoute = (href: string) => (e: MouseEvent) => {
    if (href.startsWith('#')) {
      e.preventDefault();
      ...
      <div className="bg-black text-white text-sm flex flex-col sm:flex-row justify-between items-center px-4 py-2 gap-2">
        ...
        <div className="flex gap-3 items-center">
          ...
          <Link
            ...
            className="inline-flex items-center gap-1 rounded-md bg-white/10 px-3 py-1.5 text-sm font-medium text-white hover:bg-white/20 transition"
            title="Admin Login"
            onClick={handleHashOrRoute('/billing/login')}
          >
            <span aria-hidden="true">🔒</span>
            <span>Admin Login</span>
          </Link>
  ...
  <Link href="/"         className={itemClassDesktop('/')}>HOME</Link>
  <Link href="/about"    className={itemClassDesktop('/about')}>ABOUT US</Link>
  <Link href="/services" className={itemClassDesktop('/services')}>SERVICES</Link>
  <Link href="/countries"className={itemClassDesktop('/countries')}>COUNTRIES</Link>
  <Link href="/contact"  className={itemClassDesktop('/contact')}>CONTACT US</Link>
  <Link href="/counselling" className="ml-3 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white px-4 py-1.5 rounded-full font-medium hover:scale-105 transition-all duration-300 shadow-md">
    Book Free Counselling
  </Link>
</div>
      ...
      {/* Mobile Drawer — full-width like image #2 */}
{menuOpen && (
  <div className="lg:hidden fixed inset-0 z-[60]">
    {/* Backdrop */}
    <div
    ...
    {/* Full-width panel */}
    <div className="relative w-full max-w-none rounded-none border-0 bg-slate-900/95 shadow-xl">
      {/* Top row: logo + close */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
        <Image
          src="/Studyvisummp4logo (1) (1).gif"
      ...
      {/* Links */}
<div className="px-3 py-3 space-y-2 lg:hidden">
  <Link href="/"         onClick={handleHashOrRoute('/')}         className={itemClassMobile('/')}>HOME</Link>
  <Link href="/about"    onClick={handleHashOrRoute('/about')}    className={itemClassMobile('/about')}>ABOUT US</Link>
  <Link href="/services" onClick={handleHashOrRoute('/services')} className={itemClassMobile('/services')}>SERVICES</Link>
  <Link href="/countries"onClick={handleHashOrRoute('/countries')}className={itemClassMobile('/countries')}>COUNTRIES</Link>
  <Link href="/contact"  onClick={handleHashOrRoute('/contact')}  className={itemClassMobile('/contact')}>CONTACT US</Link>
        ...
        <Link
          href="/counselling"
          onClick={handleHashOrRoute('/counselling')}
          className="block text-center bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg font-medium mt-2"
        >
          Book Free Counselling
        </Link>
